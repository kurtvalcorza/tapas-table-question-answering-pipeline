...
MARKDOWN_MARKERS = (
    "**Capability:** table question answering (one table of string cells + one question → selected cells, one aggregation operator NONE/SUM/AVERAGE/COUNT, and a pipeline-computed numeric answer)",
    ...
    "**Decision semantics:** the cell decision is a fixed sigmoid threshold (`CELL_THRESHOLD` = 0.5) and the operator decision is an `argmax` over four logits",
    "**The numeric answer is the pipeline's arithmetic**, not a model output",
    "**`denotation_accuracy`** — the WikiTableQuestions criterion",
    "the verdict is **`sample-sanity`** with a single-sample estimate and no dispersion",
    "so the verdict is `not-measurable` and the report states what would make it measurable",
    "**truncated** by the tokenizer (cell text trimmed to a common token count, then trailing rows dropped)",
    "Every header and cell is a **string**: TAPAS tokenises text",
    "conversational or multi-turn table QA (the SQA setting)",
    "**Input/output contract.** `answer(table, query)`",
)
